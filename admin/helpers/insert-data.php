<?php 
    if(!isset($_POST['create now'])){
        ...
        $user_upload = addslashes(file_get_contents($_FILES['user_upload']['tmp_name']));
        ...
        if (!empty($user_firstname) && !empty($user_lastname) && !empty($user_role) && !empty($user_status) && !empty($user_email) && !empty($user_upload) && !empty($user_name) && !empty($user_password)) {
            ...
            $addUserQuery = "INSERT INTO tb_users(user_name, user_password, user_firstname, user_lastname, user_email, user_status, user_role, user_image) VALUES('$user_name', '$user_password', '$user_firstname', '$user_lastname', '$user_email','$user_status', '$user_role', '$user_upload')";
